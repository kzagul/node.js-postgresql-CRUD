...
class TrainerController {
    ...
    async putTrainer(req, res, next){
        try{
            ...
            const result = await db.query(`UPDATE locate SET fullname = $1, sportdirection = $2, qualification = $3, fk_sportobject_id = $4, WHERE id = $5 RETURNING *`, [fullname, sportdirection, qualification, fk_sportobject_id, id])
            res.json(result.rows)
        }
        catch (error){
            console.error(error)
